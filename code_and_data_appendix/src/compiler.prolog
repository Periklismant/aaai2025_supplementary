...
compileED(EventDescription, DependencyGraphFile, EventsFlag):-
	...
	cleanCache.
%
compileED(EventDescription, DependenciesFile, DependencyGraphFile, EventsFlag):-
	% compile the event description specified with <ApplicationName>.
	compileApplication(EventDescription),
    % write dependencies, cdc, cdcedge, cdcLevel.
    writeDependencies(DependenciesFile),
	% create dependency graph the event description.
	createDependencyGraphTxt(DependencyGraphFile, EventsFlag),
	% retract facts asserted by the compiler. 
	cleanCache.

compile(EventDescription, DependenciesFile):-
    compileED(EventDescription, DependenciesFile, 'dependency_graph.dot', withEvents).

writeDependencies(DependenciesFile):-
    %findall(Dependency, (dependency(Dependency), write(DependenciesFile, dependency(Dependency))), _),
    tell(DependenciesFile),
    findall(CDC, (cdc(CDC), write(vertex(CDC)), write('.'), nl), _),
    findall((V1,V2), (cdedge(V1,V2), write(edge(V1,V2)), write('.'), nl), _),
    findall(CDCLevel, (cdcLevel(CDC,CDCLevel), write(level(CDC,CDCLevel)), write('.'), nl), _),
    told.
...
writeCompiledRule(initiatedAtTest, [U, T], Body) :-
	write('initiatedAt('), write(U), write(', '), write(T), write(') :-'), nl, 
	writeBodyLiterals(Body).

writeCompiledRule(terminatedAtTest, [U, T], Body) :-
	write('terminatedAt('), write(U), write(', '), write(T), write(') :-'), nl, 
	writeBodyLiterals(Body).
