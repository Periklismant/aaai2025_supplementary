...
function set_prolog_command() {
	...
	[ -z $interactive_mode ] && prolog_command="swipl --stack-limit=4g -l continuousQueries.prolog -g continuousQueries(${application},${second_argument}),halt." || prolog_command="swipl -l continuousQueries.prolog -g continuousQueries(${application},${second_argument})."
}
...
function socket_writer (){
    ...
    # Loop until a socket appears in path <socket>
    until [ -S $socket ]
    do 
        :
    done
...
# the socket client reads from the csv files and write the events into the socket,
